 ...
 */
  getAllOutputFromApi: function () {
    var that = this;
    wx.request({
      url: app.globalData.WebUrl + "/project/chartcollect/list?startDate=" + that.data.startDate + "&endDate=" + that.data.endDate,
      method: 'GET',
      // 设置请求的 header  
      header: {
      ...
      success: function (res) {
        if (res.statusCode == 200) {
          let outPutNumSum = 0;
          let projectSum = 0;
          for(let dat of res.data.list){
            outPutNumSum += dat.output;
            projectSum += dat.projectNum;
          }
